...
function generateFixSuggestion(
  ...
  if (issueLower.includes("secret") || issueLower.includes("hardcoded") || issueLower.includes("password")) {
    return {
  ...
  if (issueLower.includes("console.log") || issueLower.includes("console")) {
    return {
      ...
      replacement: snippet.replace(/console\.(log|warn|error|debug)\s*\(([^)]*)\)/, "ctx.logger.info($2)"),
  ...
  return {
    ...
    explanation: "Apply standard best practices: validate inputs, handle errors, follow language conventions.",
  };
}
...
const reviewPr: Tool = {
  definition: {
    ...
    params: [
      { name: "repo", type: "string", description: "GitHub repository in owner/name format", required: true },
      { name: "pr_number", type: "number", description: "Pull request number to review", required: true },
      { name: "post_comments", type: "boolean", description: "Whether to post review comments", required: false },
      { name: "severity", type: "string", description: "Minimum severity to report", required: false, enum: ["info", "warning", "error", "critical"] },
    ],
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.repo || typeof args.repo !== "string" || !args.repo.includes("/")) {
        return { toolName, success: false, output: "", error: "repo must be in 'owner/name' format", durationMs: Date.now() - start };
      }
      const prNumber = args.pr_number;
      if (typeof prNumber !== "number" || prNumber < 1) {
        return { toolName, success: false, output: "", error: "pr_number must be a positive integer", durationMs: Date.now() - start };
      }
      ...
      if (!config.githubToken) {
        return { toolName, success: false, output: "", error: "GitHub token not configured. Set githubToken in plugin settings.", durationMs: Date.now() - start };
      }

      // Fetch PR diff from GitHub
      const diffUrl = `https://api.github.com/repos/${repo}/pulls/${prNumber}`;
      const diffResponse = await fetch(diffUrl, {
        headers: { ...getGitHubHeaders(config.githubToken), Accept: "application/vnd.github.v3.diff" },
          ...
          for (const line of file.addedLines) {
            for (const pattern of cat.patterns) {
              if (pattern.test(line.content)) {
                const severity = catKey === "security" ? "error" : catKey === "bugs" ? "warning" : "info";
      ...
      if (postComments) {
        await postGitHubReview(repo, prNumber, { prNumber, repo, totalFiles: files.length, totalFindings: allFindings.length, findings: allFindings, summary, recommendation });
      }

      return {
        toolName, success: true,
        output: JSON.stringify({ prNumber, repo, totalFiles: files.length, totalFindings: allFindings.length, findings: allFindings, summary, recommendation }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
...
const analyzeDiff: Tool = {
  definition: {
    ...
    params: [
      { name: "diff", type: "string", description: "Raw git diff content to analyze", required: true },
      { name: "language", type: "string", description: "Primary programming language", required: false },
    ],
    capabilities: [],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.diff || typeof args.diff !== "string") {
        return { toolName, success: false, output: "", error: "diff must be a non-empty string", durationMs: Date.now() - start };
      }
        ...
        for (const [catKey, cat] of Object.entries(CHECK_CATEGORIES)) {
          for (const line of file.addedLines) {
            for (const pattern of cat.patterns) {
              if (pattern.test(line.content)) {
                findings.push({
                  file: file.filename, line: line.newLineNumber,
                  severity: catKey === "security" ? "error" : catKey === "bugs" ? "warning" : "info",
                  category: cat.name,
      ...
      return {
        toolName, success: true,
        output: JSON.stringify({ files: files.length, findings: findings.length, findings, language: args.language }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
...
const checkSecurity: Tool = {
  definition: {
    name: "check_security",
    description: "Deep security analysis of code content. Checks for hardcoded secrets, unsafe patterns.",
    params: [
      { name: "content", type: "string", description: "Code content to analyze", required: true },
      { name: "file_path", type: "string", description: "Path of the file being analyzed", required: false },
    ],
    capabilities: [],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const secPatterns: [string, RegExp, string][] = [
        ["Hardcoded secret", /(password|passwd|secret|token|api[_-]?key|auth)\s*[=:]\s*["'][^"'\s]{4,}["']/i, "Move secrets to environment variables."],
        ...
        ["dangerouslySetInnerHTML", /dangerouslySetInnerHTML/, "This React prop bypasses XSS protection."],
        ["Weak crypto", /MD5|SHA-?1\b/i, "Use SHA-256 or stronger hashing."],
        ["SQL injection risk", /("|')\s*\+\s*\w+\s*\+\s*("|')/, "Use parameterized queries."],
        ["Hardcoded URL with credentials", /https?:\/\/[^\s"']*password[^\s"']*/i, "URL contains credentials."],
      ];
        ...
        for (const [name, pattern, suggestion] of secPatterns) {
          if (pattern.test(lines[i])) {
            findings.push({
              ...
              severity: name.includes("Hardcoded") || name.includes("injection") ? "critical" : "error",
              category: "Security",
      ...
      return {
        toolName, success: true,
        output: JSON.stringify({ file: filePath, lines: lines.length, findings: findings.length, findings }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!args.language || typeof args.language !== "string") {
        return { toolName, success: false, output: "", error: "language must be a non-empty string", durationMs: Date.now() - start };
      }
      ...
      const styleChecks: Record<string, [string, RegExp, string][]> = {
        typescript: [
          ...
          ["console.log left in code", /console\.(log|warn|debug)\s*\(/, "Remove debugging console statements."],
      ...
      for (let i = 0; i < lines.length; i++) {
        for (const [name, pattern, suggestion] of rules) {
          if (pattern.test(lines[i])) {
            findings.push({
              file: "unknown", line: i + 1, severity: "info",
              category: "Style",
      ...
      return {
        toolName, success: true,
        output: JSON.stringify({ language, lines: lines.length, findings: findings.length, findings }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
...
const suggestFixes: Tool = {
  definition: {
    ...
    params: [
      { name: "code_snippet", type: "string", description: "The code snippet that needs fixing", required: true },
      { name: "issue_description", type: "string", description: "Description of the issue to fix", required: true },
    ],
    capabilities: [],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.code_snippet || typeof args.code_snippet !== "string") {
        return { toolName, success: false, output: "", error: "code_snippet must be a non-empty string", durationMs: Date.now() - start };
      }
      if (!args.issue_description || typeof args.issue_description !== "string") {
        return { toolName, success: false, output: "", error: "issue_description must be a non-empty string", durationMs: Date.now() - start };
      }
...
const reviewChecks: Tool = {
  definition: {
    ...
    params: [
      { name: "action", type: "string", description: "Action to perform", required: false, enum: ["list", "run"] },
      { name: "categories", type: "string", description: "Comma-separated categories to run", required: false },
    ],
    capabilities: [],
  },
    ...
    try {
      ...
      if (action === "list") {
        const categories = Object.entries(CHECK_CATEGORIES).map(([key, cat]) => ({
          id: key, name: cat.name, description: cat.description, patternCount: cat.patterns.length,
        }));
        return {
          toolName, success: true,
      ...
      return {
        toolName, success: true,
        output: JSON.stringify({ action, categories: args.categories, message: "Run category analysis using review_pr or analyze_diff." }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
...
async function postGitHubReview(repo: string, prNumber: number, result: ReviewResult): Promise<void> {
  ...
  const comments = result.findings
    ...
    .map((f) => ({
      path: f.file,
      line: f.line,
      body: `**${f.severity.toUpperCase()}**: ${f.message}\n\nSuggestion: ${f.suggestion || "Review this finding."}`,
    }));

  const body = {
    body: result.summary,
    event: result.recommendation === "approve" ? "APPROVE" : result.recommendation === "request_changes" ? "REQUEST_CHANGES" : "COMMENT",
    comments,
  };
  ...
  try {
    ...
    console.error(`[pr-review] GitHub review post failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
